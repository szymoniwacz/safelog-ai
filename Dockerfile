...
# check=error=true
#
# Production / Fly.io runtime only. Pin versions via ARG below — do NOT install or use mise here.
# Local development uses mise (.mise.toml) for Ruby and Node; keep toolchains separate.
...
# Thruster listens on HTTP_PORT (default 80); non-root cannot bind :80 on Fly.
